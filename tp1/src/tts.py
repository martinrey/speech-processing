...
from parsers import PitchParser
import plotly
...
def main():
    ...
    while i < len(difonos):
        if (pregunta and 'A' in difonos[i]):
            ...
            os.system(
                'praat scripts/extraer-pitch-track.praat ../audios/difonos/' + str(difonos[i]) + '.wav ../tmp/' + str(
                    difonos[i]) + '.PitchTier 50 300')
            # Modifico Pitch
            pitch_parser = PitchParser()
            pitch_parser.parse(filename="tmp/" + str(difonos[i]) + '.PitchTier')
            # Resintetizamos audio
            os.system('praat scripts/reemplazar-pitch-track.praat ../audios/difonos/' + str(
                difonos[i]) + '.wav ../tmp/' + str(difonos[i]) + '.PitchTier ../tmp/' + str(difonos[i]) + '.wav 50 300')
    ...
    if (pregunta):
        # Extraemos pitch
        os.system('praat scripts/extraer-pitch-track.praat ' + str(output_dir) + " ../tmp/" + str(
            output_name) + '.PitchTier 50 300')
        # Modificamos pitch
        pitch_parser = PitchParser()
        pitch_parser.parse(filename="tmp/" + str(output_name) + '.PitchTier')
        # Resintetizamos audio
        os.system('praat scripts/reemplazar-pitch-track.praat ' + str(output_dir) + ' ../tmp/' + str(
            output_name) + '.PitchTier '
